...
function rateFor(l, Y, m, today) {
  ...
  // Governing rate: each rentSchedule segment is the contractual rent for its
  // period (flat). Beyond the latest segment, escalate from THAT segment's
  // effective date so a known future step (auto-renewal at a stated rate) isn't
  // double-counted.
  if (l.rentSchedule && l.rentSchedule.length) {
    const pd = (s) => { const p = String(s).split("-"); return new Date(+p[0], (+p[1] || 1) - 1, +p[2] || 1); };
    let gov = null, cur = null;
    for (const seg of l.rentSchedule) { const ef = pd(seg.effective);
      if (!cur || ef > pd(cur.effective)) cur = seg;
      if (ef <= ms && (!gov || ef > pd(gov.effective))) gov = seg; }
    if (gov) {
      const gbase = Number(gov.monthlyRent) || 0;
      if (gov !== cur) return gbase;
      if (l.escalationPct && l.escalationMonth) {
        const ce = pd(cur.effective); let cc = 0;
        for (let yy = ce.getFullYear(); yy <= ms.getFullYear(); yy++) { const ee = new Date(yy, l.escalationMonth - 1, 1); if (ee > ce && ee <= ms) cc++; }
        return gbase * Math.pow(1 + l.escalationPct / 100, cc);
      }
      return gbase;
    }
  }
  ...
  if (l.escalationPct && l.escalationMonth) {
    // `rent` is the rent in effect as of `today`. Step it by the NET number of
    // escalation anniversaries (escalationMonth/1) between `today` and this
    // month: positive for future months, negative for earlier months of the
    // current year that predate an escalation already taken this year.
    // Escalations only begin the year AFTER commencement, so skip any
    // anniversary in the commencement year or earlier.
    const cYear = l.commenced ? parseInt(String(l.commenced).slice(0, 4), 10) : -Infinity;
    let lo, hi, sign; if (ms >= today) { lo = today; hi = ms; sign = 1; } else { lo = ms; hi = today; sign = -1; }
    let c = 0; for (let y = lo.getFullYear(); y <= hi.getFullYear(); y++) { if (y <= cYear) continue; const ed = new Date(y, l.escalationMonth - 1, 1); if (ed > lo && ed <= hi) c++; }
    return rent * Math.pow(1 + l.escalationPct / 100, sign * c);
  }
  return rent;
}

// Percent abated (0-100) for a month overlapping a rent-abatement window.
function abatePctFor(abs, ms) {
  if (!abs || !abs.length) return 0;
  const pd = (s) => { const p = String(s).split("-"); return new Date(+p[0], (+p[1] || 1) - 1, +p[2] || 1); };
  const mEnd = new Date(ms.getFullYear(), ms.getMonth() + 1, 0);
  for (const a of abs) { const f = pd(a.from), t = a.to ? pd(a.to) : f; if (f <= mEnd && t >= ms) return a.pct != null ? Number(a.pct) : 100; }
  return 0;
}
// Projected base rent for one lease in a given month, net of rent abatements.
function monthRentFor(l, Y, m, today) {
  let base = rateFor(l, Y, m, today);
  if (base && l.abatements && l.abatements.length) {
    const ab = abatePctFor(l.abatements, new Date(Y, m, 1));
    if (ab) base = base * (1 - ab / 100);
  }
  return base;
}
...
export function tenant_contact(args, ctx) {
  const q = args.tenant || "";
  const shape = (c) => ({ name: c.name || null, title: c.title || null, email: c.email || null, phone: c.phone || null,
    roles: c.roles && c.roles.length ? c.roles : undefined, broker: c.isBroker || undefined });
  const byPrimary = (a, b) => (b.isPrimary - a.isPrimary) || (a.name || "").localeCompare(b.name || "");

  // Candidate A: best current tenant by name.
  let tBest = null, tScore = 0;
  for (const t of ctx.E.tenants || []) { const s = fuzzy(q, t.name || ""); if (s > tScore) { tScore = s; tBest = t; } }
  // Candidate B: best contact-group label (covers prospects / former tenants whose
  // contacts have no current crosby tenantId, e.g. "Edwards Jones", "Goodreau").
  const groups = {};
  for (const c of ctx.E.contacts || []) { const g = c.group || ""; if (g) (groups[g] = groups[g] || []).push(c); }
  let gBest = null, gScore = 0;
  for (const g of Object.keys(groups)) { const s = fuzzy(q, g); if (s > gScore) { gScore = s; gBest = g; } }

  // Prefer a contact group only when it matches the query strictly better than any
  // current tenant — otherwise a loose token (e.g. "Jones") could hijack the match.
  if (gBest && gScore > tScore) {
    const cs = groups[gBest].slice().sort(byPrimary);
    const status = cs[0].status || null;
    return { tenant: gBest, status, contactCount: cs.length, contacts: cs.map(shape),
      note: status && status !== "Active" ? `Note: "${gBest}" is marked ${status} in our records (not a current tenant).` : undefined };
  }
  if (tBest) {
    const cs = ((ctx.contactsByTenant && ctx.contactsByTenant.get(tBest.id)) || []).slice().sort(byPrimary);
    if (cs.length) return { tenant: tBest.name, contactCount: cs.length, contacts: cs.map(shape) };
    if (tBest.contactName || tBest.email || tBest.phone)
      return { tenant: tBest.name, contactCount: 1, contacts: [{ name: tBest.contactName || null, email: tBest.email || null, phone: tBest.phone || null }] };
    return { tenant: tBest.name, contacts: [], note: "No contact details are on file for this tenant yet." };
  }
  return { note: `No tenant or contact group found matching "${q}".` };
}
...
// Door / alarm / key access instructions for the office buildings. Filter by
// building, suite, tenant, or a free-text query (e.g. "front door", "alarm",
// "clubhouse pool"). Returns the codes/keys verbatim.
function access_instructions(args, ctx) {
  let rows = (ctx.E.accessInstructions || []).slice();
  const prop = args.property ? propMatch(args.property, ctx) : null;
  if (prop) rows = rows.filter((a) => !a.propertyId || a.propertyId === prop.id);
  if (args.building) rows = rows.filter((a) => a.building && buildingMatches(args.building, a.building));
  if (args.suite) {
    const s = String(args.suite).toLowerCase().replace(/suite|ste\.?|\s/gi, "");
    rows = rows.filter((a) => a.suite && String(a.suite).toLowerCase().replace(/\s/g, "").includes(s));
  }
  if (args.tenant) rows = rows.filter((a) => a.tenant && fuzzy(args.tenant, a.tenant) >= 50);
  if (args.query) {
    const q = String(args.query).toLowerCase();
    rows = rows.filter((a) => [a.label, a.tenant, a.notes, a.area, a.keyDescription].filter(Boolean).join(" ").toLowerCase().includes(q));
  }
  rows = rows.sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0)).slice(0, 80);
  return {
    count: rows.length,
    note: "Physical access details (door/alarm/key codes). Share only with authorized staff.",
    access: rows.map((a) => ({
      where: a.building ? `${a.building}${a.level ? ` Level ${a.level}` : ""}${a.suite ? ` Suite ${a.suite}` : ""}` : (a.area === "clubhouse" ? "Clubhouse" : "Campus-wide"),
      label: a.label, tenant: a.tenant || null, method: a.method || null,
      code: a.code || null, altCode: a.altCode || null, key: a.keyDescription || null, notes: a.notes || null,
    })),
  };
}
...
const REGISTRY = { find_tenant, get_lease_documents, building_or_property_rent, portfolio_totals, property_summary, expiring_leases, vacancies, insurance_status, monthly_rent_roll,
  next_rent_increase, lease_action, notice_deadlines, building_directory, space_search, rent_per_sf, occupancy_outlook, portfolio_kpis, tenant_contact, suite_history, access_instructions };
...
export const TOOL_DEFS = [
  ...
  { type: "function", function: { name: "tenant_contact", description: "All contacts on file for a tenant — each with name, title, email, phone, and roles (Primary/Secondary/Leasing/Billing; brokers flagged). Also resolves prospect/former-tenant contact groups by name.", parameters: { type: "object", properties: { tenant: { type: "string" } }, required: ["tenant"] } } },
  ...
  { type: "function", function: { name: "access_instructions", description: "Physical building access: door codes, alarm codes, keys, scan-card/fob, and special entry notes for Sanctuary Office Park buildings, suites, common areas and the Clubhouse. Use for any 'how do I get in', 'what's the code/key for', door/alarm/keypad/gate questions. Filter by building, suite, tenant, or a free-text query.", parameters: { type: "object", properties: { building: { type: "string" }, suite: { type: "string" }, tenant: { type: "string" }, property: { type: "string" }, query: { type: "string", description: "free-text, e.g. 'front door', 'alarm', 'clubhouse pool', 'bathroom'" } } } } },
];
